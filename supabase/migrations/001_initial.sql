-- PolSite Database Schema
-- Privacy-first: no individual demographic data stored, only aggregate tallies
...
-- Votes: stores user_id for double-vote prevention, NO demographic data
CREATE TABLE votes (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Vote demographics: AGGREGATE TALLIES ONLY - no user_id, no individual records
...
-- Vote anomalies: flagged suspicious patterns for admin review
CREATE TABLE vote_anomalies (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  poll_id UUID NOT NULL REFERENCES polls(id) ON DELETE CASCADE,
  anomaly_type TEXT NOT NULL, -- 'ip_cluster', 'rapid_creation', etc.
...
-- Atomic vote + demographic tally increment
-- Individual demographic data exists ONLY in function memory, never persisted
...
BEGIN
  ...
  -- Aggregate demographic tally (if provided)
...
-- Votes: users can see own votes only
CREATE POLICY "Users can see own votes"
  ON votes FOR SELECT USING (auth.uid() = user_id);
-- Votes are inserted via the cast_vote_with_demographics function (SECURITY DEFINER)

-- Vote demographics: anyone can read aggregates
CREATE POLICY "Anyone can read demographic tallies"
  ON vote_demographics FOR SELECT USING (true);
-- No INSERT/UPDATE/DELETE from client - only the server function writes
